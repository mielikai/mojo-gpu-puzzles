...
#        partial_product = rebind[Scalar[dtype]](a[global_i]) * rebind[Scalar[dtype]](b[global_i])

    # Block approach: 1 line across multiple warps (128+ threads)
    var total = block.sum[block_size=tpb, broadcast=False](partial_product)
...
def block_histogram_bin_extract[
    ...
    size: Int,
    target_bin: Int, # current bin - block_histogram_bin_extract() is called once for each bin value
    num_bins: Int,
):
   ...
   # Step 1: Each thread determines its bin and element value
    ...
    ](val=SIMD[DType.int32, 1](Float32(belongs_to_target)))

    # Step 4: Extract and pack elements belonging to target_bin
    if belongs_to_target == 1:
        bin_output[Int(write_offset[0])] = my_value # a call to block_histogram_bin_extract() overwrites previous element values in bin_output
...
def block_normalize_vector[
    ...
    # Step 2: Use block.sum() to compute total sum
    ...
    # Step 4: block.broadcast() shares mean from thread 0 to ALL threads
    ...
    # Step 5: Each thread normalizes by the broadcasted mean
    if global_i < size:
        var normalized_value = my_value / broadcasted_mean
